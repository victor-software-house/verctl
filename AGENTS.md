...
`.miserc.toml` defaults local `MISE_ENV` to `dev`. `[[pins]]` in
`verctl.toml` rewrites those tool entries on the Version PR when
the named package bumps. Actions run `verctl` from PATH.
